...
struct pci_ll *pci_ll_head = NULL, *pci_ll_tail = NULL;

struct pci_ll *bridges[256] = {};
// size_t bridge_idx = 0;
...
bool probe_pci_device(paddr_t base, uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset) {
    ...
    if (device_header->device_id == 0xffff) return false;
    // printf("Slot #%d@0x%p:"
    ...
    if (device_header->class_code == 0x06) {
        ...
        pci_ll_tail = bridge;
        if (pci_ll_head == NULL) pci_ll_head = bridge;
        bridge->device.pci = device_header;

        if (slot == 0) bridges[bus] = bridge;
    } else {
        ...
        if (bridge->first_child == NULL) bridge->first_child = device;
        else {
            for (; ; tail = tail->next) if (tail->next == NULL) break;
            tail->next = device;
        }
...
void probe_pci(paddr_t base) {
    if (sizeof(struct pci_ll) > 16) PANIC("PCI Linked List entry is too big for a slab16 (%d > 16)!\n", sizeof(struct pci_ll));
    ...
    for (size_t i = 0; i < 256 && bridges[i] != NULL; i++) {
        ...
        switch (bridges[i]->device.pci->subclass) {
            case 0x00: bridge_type = "Host"; break;
            default: bridge_type = "unkown-type"; break;
        }
        switch (bridges[i]->device.pci->vendor_id) {
            case 0x1b36: vendor_id = "Red Hat, Inc."; break;
            default: vendor_id = "unkown-vendor"; break;
        }
        switch (bridges[i]->device.pci->device_id) {
            case 0x0008: device_id = "QEMU PCIe Host bridge"; break;
            default: device_id = "unkown-device"; break;
        }
        kprintf("[Bus %d] [%x:%x] %s Bridge (%s %s).\n", i, bridges[i]->device.pci->vendor_id, bridges[i]->device.pci->device_id, bridge_type, vendor_id, device_id);
            ...
            switch (child->device.pci->class_code) {
                ...
                case 0x1: {
                    switch (child->device.pci->subclass) {
                        case 0x0: bridge_type = "SCSI bus controller"; break;
                        case 0x1: { 
                            // TODO: switch on progIF
                            bridge_type = "IDE controller";
                         } break;
                        case 0x2: bridge_type = "Floppy disk controller"; break;
                        case 0x3: bridge_type = "IPI bus controller"; break;
                        case 0x4: bridge_type = "RAID contoller"; break;
                        case 0x5: {
                            // TODO: switch on progIF
                        ...
                        default: bridge_type = "Generic mass storage controller"; break;
                    }
                };
                case 0x03: {
                    switch (child->device.pci->subclass) {
                        case 0x0: bridge_type = "VGA display controller"; break;
                        case 0x1: bridge_type = "XGA display controller"; break;
                        case 0x2: bridge_type = "3D display controller (non-VGA)"; break;
                        default: bridge_type = "Generic display controller"; break;
                    }
                } break;
                default: bridge_type = "Unknown device"; break;
            }
            switch (child->device.pci->vendor_id) {
                case 0x1234: vendor_id = "BOCHS"; break;
                default: vendor_id = "unkown-vendor"; break;
            }
            switch (child->device.pci->device_id) {
                case 0x1111: device_id = "Graphics Adaptor"; break;
                default: device_id = "unkown-device"; break;
            }
            kprintf("   | [%x:%x] %s (%s %s)\n", child->device.pci->vendor_id, child->device.pci->device_id, bridge_type, vendor_id, device_id);
        }
    }
    // PANIC("done");
    // while(true){}
